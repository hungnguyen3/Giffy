...
export const createCollection = async (req: any, res: any) => {
  ...
  try {
    ...
    if (createCollectionsUsersRes.rowCount !== 1) {
      ...
      return res.status(404).json({ error: "couldn't create a relationship between collection & user" });
    };
...
export const deleteCollectionById = async (req: any, res: any) => {
  await client
    ...
    .then((dbRes: any) => {
      if (dbRes.rowCount === 0) res.status(404).send("There is no such collection");

      if (dbRes.rowCount === 1) {
        res
          .status(200)
          .send(
            "you have successfully deleted a collection uid: " + req.params.collectionId
          );
      }
    ...
    .then((dbRes: any) => {
      ...
      if (dbRes.rowCount === 1) {
        res
          .status(200)
          .send(
            "you have successfully update a collection uid: " + req.params.collectionId
          );
      }
